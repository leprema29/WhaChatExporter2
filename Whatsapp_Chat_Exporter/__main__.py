...
def setup_argument_parser() -> ArgumentParser:
    ...
    plugin_group = parser.add_argument_group('Export Format Plugins')
    plugin_group.add_argument(
        "--markdown", dest="markdown_export", nargs='?', default=None, type=str, const="result",
        help="Export chats in Markdown format (default if present: result/)"
    )
    plugin_group.add_argument(
        "--csv", dest="csv_export", nargs='?', default=None, type=str, const="result",
    ...
    config_group.add_argument(
        "--generate-config", dest="generate_config", nargs='?', default=None, type=str,
        const="wtsexporter.toml",
        help="Generate a sample configuration file"
    )

    # Chat name filtering
    filter_group.add_argument(
        "--chat-name", dest="filter_chat_names", nargs='*', metavar="name",
        help="Filter chats by name (partial match supported)"
    )

    # Security / Anonymization
    security_group = parser.add_argument_group('Security')
    security_group.add_argument(
        "--anonymize", dest="anonymize", default=False, action='store_true',
        help="Anonymize personal data (names, phone numbers) in the export"
    )

    # Overview page
    output_group.add_argument(
        "--overview", dest="generate_overview", default=False, action='store_true',
...
def generate_overview_page(data: ChatCollection, output_folder: str) -> None:
    """Generate an HTML overview page with all chat summaries."""
    import jinja2
    ...
    for chat_id, chat in data.items():
        ...
        # Count media
        media_count = sum(1 for m in chat.values() if m.media)
        ...
        if last_msg.data and not last_msg.meta:
            text = last_msg.data.replace("<br>", " ").replace(" <br>", " ")
            last_message_text = text[:80] + "..." if len(text) > 80 else text

        # Build timeline
        for msg in chat.values():
            month = datetime.fromtimestamp(msg.timestamp).strftime("%Y-%m")
            monthly_counts[month] += 1

        # Determine chat type
        if chat_id.endswith("@g.us") or "-" in chat_id.split("@")[0]:
            chat_type = "group"
        else:
            chat_type = "personal"
        ...
        chats_info.append({
            ...
            "type": chat_type,
            "avatar": chat.their_avatar_thumb or chat.their_avatar,
        })

    # Sort by most recent
    chats_info.sort(key=lambda x: x["last_timestamp"], reverse=True)

    # Build timeline (last 12 months)
    sorted_months = sorted(monthly_counts.keys())
    timeline = [(m, monthly_counts[m]) for m in sorted_months[-24:]] if sorted_months else []
    max_timeline = max((c for _, c in timeline), default=1)

    # Date range
    if all_timestamps:
        oldest = datetime.fromtimestamp(min(all_timestamps)).strftime("%Y-%m-%d")
        newest = datetime.fromtimestamp(max(all_timestamps)).strftime("%Y-%m-%d")
    ...
    logging.info(f"Overview page generated: {output_path}")


def filter_by_chat_names(data: ChatCollection, names: list) -> None:
    """Filter chats by name (partial match). Remove non-matching chats."""
    if not names:
        return

    to_remove = []
    for chat_id, chat in data.items():
        chat_name = (chat.name or "").lower()
        if not any(n.lower() in chat_name for n in names):
            to_remove.append(chat_id)

    for chat_id in to_remove:
        data.remove_chat(chat_id)

    logging.info(f"Filtered to {len(data)} chats matching names: {', '.join(names)}")
...
def main():
    ...
    if args.import_json:
        ...
        if args.incremental_merge:
            ...
            if hasattr(args, 'filter_chat_names') and args.filter_chat_names:
                filter_by_chat_names(data, args.filter_chat_names)
            ...
            # Export in additional formats (plugins)
            if hasattr(args, 'markdown_export') and args.markdown_export:
                plugin = get_plugin("markdown")
                if plugin:
                    plugin.export_all(data, args.markdown_export)

            if hasattr(args, 'csv_export') and args.csv_export:
            ...
            if hasattr(args, 'generate_overview') and args.generate_overview:
                generate_overview_page(data, args.output)
